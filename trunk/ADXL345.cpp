...
void ADXL345::start(byte bw_code){
  set_bit(ADXL345_POWER_CTL_ADDRESS, ADXL345_POWER_CTL_MEAS_BIT);
  set_bit(ADXL345_DATA_FORMAT_ADDRESS, ADXL345_DATA_FORMAT_FULL_RES_BIT);
  set_bw(bw_code);
}

boolean ADXL345::stop(){
  return clear_bit(ADXL345_POWER_CTL_ADDRESS, ADXL345_POWER_CTL_MEAS_BIT);
}

boolean ADXL345::get_gs(double *xyz){
  int vals[3];
  byte i;
  boolean status;
  double gains[3] = {0.0037639,
		     0.00376009,
		     0.00349265};
  status = read_accel(vals);
  if(status){
    for(i = 0; i < 3;i++){
      xyz[i] = vals[i] * gains[i];
    }
  }
  return status;
}
